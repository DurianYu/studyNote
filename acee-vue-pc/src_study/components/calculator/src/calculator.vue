      ...
      <div slot="header" class="clearfix">
        <el-row type="flex" :align="'middle'">
          <el-col :span="3">
            <el-dropdown @command="handleCommand" trigger="click">
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item v-for="v of user_exp" :key="v">{{ v }}</el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
          </el-col>
          <el-col :span='16'>
          ...
          <el-col :span='5'>
            <el-row>
              <el-col :span="24">
                <el-button @click="saveExp()" :loading="waiting" type="success" plain size="medium" :style="s_btn">保存并使用</el-button>
              </el-col>
              <el-col :span="12">
...
export default {
  ...
  data() {
    return {
      ...
      needful: /平均分|录取分/, // 正则表达式：后台指定的必选参数 例如： 平均分
  ...
  methods: {
    ...
    saveExp() {
      ...
      // 表达式必须有 必选参数
      if (!this.regTest(this.needful, exp.join(''))) {
      ...
      this.$confirm(`确定保存并使用: ${this.typing_ary.join('')}`, '提示', {
        ...
        // 判断是否已保存该公式
        let temp_user_exp = [];
        if (this.user_exp.join('').search(this.typing_ary.join('')) === -1) {
          temp_user_exp = this.user_exp;
          if (temp_user_exp.length > 20) temp_user_exp.shift();
          temp_user_exp.push(this.typing_ary.join(''));
        };
        // axios.post('', {user_exp: temp_user_exp
        //   })
        //   .then(res => {
        //     this.waiting = false;
        //     if (res.data.status === -1) {
        //       this.cue(res.data.msg);
        //       return;
        //     }
  ...
  created() {
    this.user_exp = ['asd6+6=++=#$^平均分平均分平均分', 'zxc', 'qwe'];
    // 获取用户的表达式
    // axios.post('', {
    //     user_exp: temp_user_exp
    //   })
    //   .then(res => {
    //     this.user_exp = res.data.data;
    //   });
  }
};
</script>
