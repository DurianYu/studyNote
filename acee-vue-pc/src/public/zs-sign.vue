...
<!--         <el-col :xs="19" :sm="11" :md="7" class="qr-sign">
        ...
        <el-col :xs="17" :sm="9" :md="5" class="account-sign">
          <div class="account-content center">
            <p>卡号密码登陆</p>
            <div class="account-input">
              <el-input type="text" autocomplete="on" size="large" placeholder="请输入账号" suffix-icon="el-icon-edit" v-model="userAccount">
              </el-input>
              <el-input type="password" autocomplete="on" size="large" placeholder="请输入密码" suffix-icon="el-icon-edit" v-model="userPassword">
              </el-input>
...
export default {
  ...
  methods: {
    ...
    userLand: function () {
      ...
      axios({
        url: `${store.state.AJAX_DOMAIN}/com/login`,
        method: 'post',
        data: {
        account: userAccount,
      ...
      .then(res => {
          res = res.data;
          if (res.status === 1) {
            store.setUserInfoAction(res.data); // 存储个人信息
            if (res.data.login === 1) {
              store.setUserInfoAction({
                province: res.data.province
              });
              // store.setUserProvinceAction(res.data.province);
              store.setUserInfoAction({...res.data, logined: true});
    ...
    getQRimg: function () {
      return new Promise((resolve, reject) => {
              axios({
        url: `${store.state.AJAX_DOMAIN}/com/qrcode`,
        method: 'post',
        data: {},})
          .then(res => {
            res = res.data;
    ...
    qrSetTime: function () {
      this.getQRimg().then(res => this.QrCodeLogin()).catch(() => {
        setTimeout(() => {
          this.getQRimg().then(res => this.QrCodeLogin())
        }, 5000);
      });
    },
    QrCodeLogin: function () {
      this.loginInter = setInterval(() => {
        axios({
          method: 'get',
          url: `${store.state.AJAX_DOMAIN}/com/qr/login`
        })
          .then ((response) => {
