<template lang="html">
  <div class="mgr-classroom-box" ref="mgr-classroom-box" style="height: -webkit-fill-available;">
      ...
      <el-row type="flex" justify="end" :gutter="12">
        <el-col :span="4">
          <el-popover
            placement="top"
            width="160"
            v-model="popover_create">
            <div style="text-align: right; margin: 0">
              <router-link :to="{name: 'mgrClassroomCreateVideo'}">
                <el-button type="primary" size="mini" @click="creVideo()">视频</el-button>
              </router-link>
              <router-link :to="{name: 'mgrClassroomCreateRecord'}">
                <el-button type="primary" size="mini" @click="creRecord()">语音</el-button>
              </router-link>
            </div>
            <el-button slot="reference">新建课堂</el-button>
          </el-popover>
          </router-link>
        </el-col>
        <el-col :span="6">
          <el-form  class="demo-form-inline">
            <el-form-item label="栏目:" style="margin: 0;">
              <el-select v-model="cur_category" @change="categoryChg">
                <el-option
    ...
    <el-table
      ...
      >
      <template slot-scope="scope">
        <el-tag :type="scope.row.type === 'record' ? 'warning' : 'success'">
          {{scope.row.type === 'record' ? '语音' : '视频'}}
        </el-tag>
      </template>
      </el-table-column>
      <el-table-column
    ...
    <el-dialog
    ref="edit-dialog"
    custom-class="classroom-edit-dialog"
    :modal="false"
...
export default {
  components: {
    CreRecord: () =>
      import('@/jg/manage/classroom-create-record'),
    CreVideo: () =>
      import('@/jg/manage/classroom-create-video')
  },
  data() {
    return {
      edit_dialog: false, // 编辑内容时 启用的dialog
      eidt_cmpt_name: '', // 编辑时 启用的组件名字
      popover_create: false, // 新建课堂进行选择用的popover
      total: 1000, // 一共多少条数据
      cur_page_num: 1, // 当前页面的页码, 初始为第一页
      h_table: '1px', // 表格的高度
      cur_category: '0', // 当前栏目的实际值
      choose_dialog: '', // 新建课堂的dialog
      main_ary: '', // 存储所有页面加载过的课堂数据
      _main_ary: '', // 存储当前页面的课堂数据
      category_data: [{
        name: '不限',
        val: '0'
      }, {
  ...
  methods: {
    ...
    // 创建视频课堂
    creVideo() {
      this.popover_create = false;
      store.setClassroomInfoAction('');
    },
    // 创建语音课堂
    creRecord() {
      this.popover_create = false;
    ...
    getIndex(id) {
      ...
      for (let i = 0, len = this.main_ary.length; i < len; i++) {
        if (String(this.main_ary[i].classroom_id) === String(id)) {
          index = i;
        }
      }
      return index;
    ...
    edit(row) {
      ...
      // 看类型打开对应的组件
      this.eidt_cmpt_name = `cre-${this.classroom_info.type}`;
      // 处理dialog宽高
      let dialog = this.$refs[`edit-dialog`].$refs['dialog'];
      let target = this.$refs['mgr-classroom-box'];
  ...
  created() {
    ...
    this._main_ary = [{
      ...
      category: '2',
      name: '初级学习',
      cover_img: `${store.state.DOMAIN}` + '/img/pc_logo.jpg'
    }];
  },
  mounted() {
    this.$nextTick(() => {
      this.h_table = this.$refs['mgr-classroom-box'].clientHeight - 73 - 60;
    });
