...
<template lang="html">
  <div class="chart-compete">
    <div class="chart-compete-content">
      <h2>
        竞争力分析图
      </h2>
...
export default {
  ...
  methods: {
    ...
    getCompeteList() {
      axios({
        url: `${store.state.AJAX_DOMAIN}/assets`,
      }).then(res => {
        res = res.data;
        if (res.status === 1) {
          this.renderData(res);
          this.painScore(res);
          // 设置该分段的总人数
          console.log(res.data.slice(-1)[0].cumulative)
