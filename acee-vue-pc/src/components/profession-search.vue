...
export default {
  ...
  methods: {
    ...
    firstInitPrfsList() {
      axiosPublicRes.get(`/json/zk-${this.major ? 'major' : 'junior'}.json`)
        .then(res => {
    ...
    setNameCode(data) {
      this.$nextTick(() => {
        ...
        if (!init_level1_text.length) {
          setTimeout(() => {
            this.setNameCode(data);
          }, 350);
          return;
        }
    ...
    clkLvlOne(code, index) {
      ...
      axiosPublicRes.get(`/json/zk-${code}.json`)
        .then(res => {
