    ...
    <section class="main">
      ...
      <Table :data="data" :columns="columns">
        <template slot-scope="{ row, index }" slot="action">
          <Button v-if="row.act == 1" type="primary" size="small" style="margin-right: 5px" @click="inspect(index)">处理</Button>
          <div v-if="row.act == 0">已处理</div>
        </template>
      </Table>
      <div class="next">
      ...
      let replace_list = [
        {
          ...
          status: '待处理',
          act: 1,
          belongStatus: [roleStatus.pay['new'][`s_pay_new_zhigu_has_online`].status, roleStatus.pay['new'][`s_pay_new_usa_has_sent_receipt`].status]
        },
        { ...this.data_template[0], status: '资金已冻结', act: 0, belongStatus: [roleStatus.pay['new'][`s_pay_new_mid_has_frozen`].status] }
      ]
