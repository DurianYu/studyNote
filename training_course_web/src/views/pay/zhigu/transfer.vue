<template lang="html">
  <div class="page">
    ...
    <section class="main font3">
      <div style="margin-left: -2rem">智谷银行</div>
      <br />
      <Form ref="formData" :model="formData" :rules="rules" :label-width="120">
        <p>王小凯账户 6217 0099 0000 5555</p>
        <br />
...
import { check_pay_zhigu_transfer } from '@/config/form-check'
...
export default {
  ...
  data() {
    const validateCheck = (rule, value, callback) => {
      return validateCheckForm(rule, value, callback, check_pay_zhigu_transfer)
    }
    return {
      formData: { up_num: '' },
      rules: {
    ...
    next() {
      ...
      this.$refs['formData'].validate(async valid => {
        if (valid) {
          ...
          await this.$store.dispatch('changeStatus', { actionid: actionCode.pay[mode][`a_pay_${mode}_zhigu_out_money`] })
          IModal.info({
            content: '账户划转成功，等待跨境支付业务处理',
            onOk() {
