...
import { check_pay_goal_tradition_transfer } from '@/config/form-check'
...
export default {
  ...
  data() {
    const validateCheck = (rule, value, callback) => {
      return validateCheckForm(rule, value, callback, check_pay_goal_tradition_transfer)
    }
    return {
      formData: {},
      rules: {
  ...
  methods: {
    next() {
      ...
      this.$refs['formData'].validate(async valid => {
        if (valid) {
          await this.$store.dispatch('changeStatus', { actionid: actionCode.pay[`old`][`a_pay_old_usa_bank_out_money`] })
          IModal.info({
            content: '账户划转成功',
            onOk() {
              _this.$router.push({ name: 'pay-goal-tradition-list' })
            }
          })
        }
      })
