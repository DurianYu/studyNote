    ...
    for v1 in os.listdir(BASIC_PATH):
        ...
        for v2 in os.listdir(os.path.join(BASIC_PATH, v1)):
            ...
            with open(json_file, 'r+', encoding=encode_type, errors='ignore') as f:
                json_data = json.loads(f.read())
                if json_data.get('_数据库信息'):
                    print('已有数据, 跳过')
                    continue
                if (count):
                    # 读取信息
                    user_login = json_data['_抖音数据']['_作者ID']
                    user_login = f'{user_login}'
                    user_pass = '###ea1e6729b1bcd5575033baf0e8cd127b'
                    user_nicename = json_data['_飞瓜数据']['_播主']
                    avatar = '/default.jpg'
                    sex = '1'
                    signature = ''
                    user_status = '1'
                    ...
                    try:
                        if is_login_id:
                            ...
                            print('-------', user_login, user_nicename)
                            # 执行sql语句
